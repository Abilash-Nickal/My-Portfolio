...
const emptyForm = { degree: "", institution: "", year: "", desc: "", imageUrl: "", skills: [], projects: [] };
...
const AdminEducation = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    // Fetch education
    const qEd = query(collection(db, "education"), orderBy("createdAt", "desc"));
    const snapEd = await getDocs(qEd);
    setItems(snapEd.docs.map((d) => ({ id: d.id, ...d.data() })));
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await updateDoc(doc(db, "education", editingId), { ...form, updatedAt: serverTimestamp() });
      } else {
        await addDoc(collection(db, "education"), { ...form, createdAt: serverTimestamp() });
      }
      await fetchData();
      setFormOpen(false);
    } finally {
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-white/20 text-sm">No education entries yet.</div>
      ) : (
          ...
          {items.map((item) => (
            <div key={item.id} className="bg-white/[0.03] border border-white/5 rounded-2xl p-5 flex items-start justify-between gap-4 hover:border-cyan-400/20 transition-all">
              <div className="flex-1 min-w-0">
                {item.year && <span className="text-[10px] font-mono text-emerald-400 tracking-widest uppercase">{item.year}</span>}
                <h3 className="text-white font-black text-base mt-0.5 mb-0.5">{item.degree}</h3>
                <p className="text-emerald-400 text-xs font-mono uppercase tracking-widest mb-2">{item.institution}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-[#0f0c18] border border-white/10 rounded-3xl p-6 w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-y-auto no-scrollbar">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-white font-black text-xl">{editingId ? "Edit Education" : "Add Education"}</h2>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { key: "degree", label: "Degree / Qualification", placeholder: "B.Sc. in Computer Science" },
                  { key: "institution", label: "Institution", placeholder: "University of Colombo" },
                  { key: "year", label: "Year / Period", placeholder: "2020 – 2024" },
                  { key: "imageUrl", label: "Institution Logo (URL)", placeholder: "https://imgur.com/logo.png" },
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="flex gap-2">
                    <button 
                      type="button" 
                      onClick={() => {
                        const textarea = document.getElementById("education-desc");
                ...
                <textarea
                  ...
                  placeholder="Describe your education... Use ### for subheadings, **bold** for bold, or - for bullets."
                  rows={6}
              ...
              <div>
                <label className="block text-xs font-bold tracking-widest uppercase mb-3 text-white/40">Academic Projects</label>
              ...
              <button type="submit" disabled={saving} className="w-full py-3 bg-gradient-to-r from-cyan-400 to-emerald-400 text-black font-black text-sm uppercase tracking-widest rounded-xl hover:shadow-[0_0_20px_rgba(34,211,238,0.4)] transition-all disabled:opacity-60 flex items-center justify-center gap-2">
                {saving ? <><Loader2 size={16} className="animate-spin" /> Saving...</> : <><Save size={16} /> Save Education</>}
              </button>
            </form>
          </div>
        </div>
          ...
          <div className="max-w-5xl mx-auto w-full h-full flex flex-col">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-white font-black text-xl tracking-tight uppercase">Detailed Education Description</h2>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="hidden sm:flex gap-2 mr-4">
                    <button 
                      type="button" 
                      onClick={() => {
                        const textarea = document.getElementById("full-education-desc");
                        const start = textarea.selectionStart;
                        const end = textarea.selectionEnd;
                        const text = form.desc;
                        const before = text.substring(0, start);
            ...
            <textarea
              id="full-education-desc"
              value={form.desc}
              onChange={(e) => setForm((f) => ({ ...f, desc: e.target.value }))}
              placeholder="Describe your education in detail... Use the toolbar above for formatting."
