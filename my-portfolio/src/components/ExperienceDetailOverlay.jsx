...
const ExperienceDetailOverlay = ({ experience, onClose, isLightMode, allSkills = [], allProjects = [], onSelectProject }) => {
  ...
  const relatedSkills = allSkills.filter((s) => {
    const isExplicitlyLinked = experience.skills?.some(id => String(id) === String(s.id) || (s.skillId && String(id) === String(s.skillId)));
    return isExplicitlyLinked;
  });

  // Filter projects associated with this experience
  const relatedProjects = allProjects.filter((p) => {
    const isExplicitlyLinked = experience.projects?.some(id => String(id) === String(p.id));
    const isLegacyMatched = p.experienceId === experience.id || p.company === experience.company;
        ...
        <div className="relative w-full md:w-[45%] h-[40vh] md:h-screen flex justify-center items-center pointer-events-auto overflow-hidden tw-fade-in tw-delay-300 order-1 flex-shrink-0 bg-gradient-to-br from-transparent to-black/5">
           {experience.imageUrl ? (
             <div className="relative group p-8 max-w-md w-full">
                <div className={`absolute inset-0 blur-3xl opacity-20 group-hover:opacity-30 transition-opacity ${isLightMode ? 'bg-orange-500' : 'bg-cyan-400'}`} />
                <img 
                  src={experience.imageUrl} 
                  alt={experience.company} 
                  className="w-full h-auto object-contain relative z-10 drop-shadow-2xl brightness-110"
                />
             </div>
           ) : (
             <Briefcase size={120} className={isLightMode ? "text-gray-200" : "text-white/10"} />
           )}
           {/* Blend gradient */}
          ...
          <div className="flex-shrink-0 p-8 md:px-16 lg:px-24 pt-20 md:pt-24 lg:pt-32 pb-6">
            <div className="tw-fade-in tw-slide-up tw-delay-300">
              <div className="flex flex-wrap gap-4 mb-4">
                <span className={`flex items-center gap-2 font-mono tracking-widest uppercase text-[10px] md:text-xs ${isLightMode ? "text-orange-500" : "text-cyan-400"}`}>
